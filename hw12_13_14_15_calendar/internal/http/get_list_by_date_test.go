...
func TestGetListByDate(t *testing.T) {
	t.Run("empty", func(t *testing.T) {
		t.Run(`"id" parsing fail`, func(t *testing.T) {
			...
			req := httptest.NewRequest("GET", "/list/date", nil)
			w := httptest.NewRecorder()

			_, err = srv.getListByDate(w, req)
			require.Error(t, err)
	...
	t.Run("errors", func(t *testing.T) {
		t.Run("can't get events list by date", func(t *testing.T) {
			t.Run("other errors", func(t *testing.T) {
				...
				req := httptest.NewRequest("GET", "/list/date?date="+now.Format("2006-01-02"), nil)
				w := httptest.NewRecorder()
	...
	t.Run("normal", func(t *testing.T) {
		...
		res, err := srv.getListByDate(w, req)
		require.NoError(t, err)
		require.EqualValues(t, getListByDate{
			EventList{
				Events: []Event{},
			},
		}, res)
